-- Opaque Predicates Layer (Advanced)
-- Features:
--   * MBA (Mixed Boolean-Arithmetic) predicates
--   * Bitwise-based predicates
--   * Multi-variable intermediate chains
--   * Nested predicates (2–5 levels)
--   * Realistic dead code: loops, state machines, closures, coroutines, pcall
--   * Wrapping modes: block / function / basic-block
--   * Sequential CFG chains: A→B→C with shared intermediate variables
--   * 100+ predicate templates to defeat pattern recognition
--   * Non-constant predicates: metatable, upvalue, closure, table-length
--   * Every run produces a different CFG
...
-- ─────────────────────────────────────────────
-- 1.  ALWAYS-TRUE PREDICATE TEMPLATES (~50+)
-- ─────────────────────────────────────────────
...
local TRUE_PREDICATES = {
    ...
    -- Identity: x - y == (x | y) - (y & x) - (y & ~x & 0xFFFF) ... simplified:
    function()
        local x = rint(10, 200); local y = rint(1, x-1)
        -- x - y == x + (~y + 1) in two's complement (truncated)
    ...
    -- Uses a local table — non-constant, harder to analyse statically
    function()
    ...
    -- ── Coroutine-based (non-constant, VM-level) ──
    function()
        local n = rint(1, 99)
        return string.format(
    ...
    function()
        ...
        return string.format(
            ...
            a,n, b,a, c,b,b, d,a, d,n)  -- careful arithmetic kept consistent
    end,
}

-- ─────────────────────────────────────────────
-- 2.  ALWAYS-FALSE PREDICATE TEMPLATES (~50+)
-- ─────────────────────────────────────────────
...
local FALSE_PREDICATES = {
    ...
    function()
        local a = rint(1,0x7F); local b = rint(1,0x7F)
        -- (a | b) != (a & b) when a!=b (almost always)
...
-- ─────────────────────────────────────────────
-- 3.  REALISTIC DEAD CODE GENERATORS
-- ─────────────────────────────────────────────
...
-- ─────────────────────────────────────────────
-- 5.  NESTED PREDICATE BUILDER  (2–5 levels)
-- ─────────────────────────────────────────────
-- Builds:   if P1 then
--               if P2 then   [if P3 then …]
--                   <live>
--               else <dead> end
--           else <dead> end
...
-- ─────────────────────────────────────────────
-- 6.  CFG CHAIN BUILDER  (A → B → C)
-- ─────────────────────────────────────────────
-- Introduces a shared intermediate variable that links
-- consecutive predicate blocks so the CFG has real data edges.

local function build_cfg_chain(lines, config)
    -- Pick chain length 2–4
    local chain_len = rint(2, 4)
    -- Shared context variable whose value threads through blocks
    local ctx_var = rvar()
    ...
    for i = 1, chain_len do
        ...
        local always_true = rbool()
        -- Predicate uses ctx_var to look non-constant
        local pred
        if i % 2 == 0 then
            -- Bitwise predicate involving ctx_var
        ...
        -- Mutate ctx_var so next block's predicate sees a "different" value
...
function OpaquePredicates.process(code, config)
    ...
    local nest          = config.nested         ~= false   -- default true
    local wrap_mode     = config.wrap_mode      or "random"
    local use_cfg_chain = config.cfg_chain      ~= false   -- default true
    ...
    while i <= #lines do
        ...
        if rfloat() < density and line:match("%S") then
            ...
            if use_cfg_chain and style_roll < cfg_prob then
                -- A→B→C chain consuming up to 3 consecutive non-blank lines
                local chunk = {}
                local j = i
            ...
            elseif nest and style_roll < (cfg_prob + 0.45) then
                -- Nested predicate (2–5 levels)
                local nested = build_nested(line, config)
                out[#out+1] = wrap(nested, wrap_mode)
                i = i + 1

            else
                -- Simple single predicate (always-true or always-false)
                local always_true = rbool()
                local pred  = pick_predicate(always_true)
                local dead  = generate_dead_code(config)
...
-- ─────────────────────────────────────────────
-- 9.  CONVENIENCE: pick_true / pick_false (for external use)
-- ─────────────────────────────────────────────
