-- variable_mangling_v2.lua (standalone)
-- Lexer + Scope Analyzer + Variable Mangling gộp chung
...
function ScopeAnalyzer.analyze(tokens)
    ...
    local var_map    = {}  -- token_index -> { scope, name }
    ...
    -- Trả về index token thực tiếp theo (bỏ WHITESPACE/COMMENT)
            ...
            elseif tok.value == "function" then
                ...
                if tokens[ni] and tokens[ni].type == "NAME" then
                    ...
                    while tokens[ci] and (tokens[ci].value == "." or tokens[ci].value == ":") do
                        ci = next_real_idx(ci)  -- bỏ field name
                        ci = next_real_idx(ci)
                    end
                    ni = ci
                end
                -- parse params: ni phải là "("
...
-- ============================================================
-- RANDOM UTILS (inline — thay RandomUtils external)
-- ============================================================
local RandomUtils = {}
...
function RandomUtils.random_variable_name(len)
    ...
    local first = CHARSET:sub(math.random(1,#CHARSET), math.random(1,#CHARSET))
    if #first == 0 then first = "a" end
    local chars = { first:sub(1,1) }
    local full  = CHARSET .. "0123456789_"
    for _ = 2, len do
        local idx = math.random(1, #full)
        table.insert(chars, full:sub(idx, idx))
    end
    return table.concat(chars)
end

-- Unicode confusable identifiers (Lua 5.3+ với UTF-8)
function RandomUtils.random_unicode_string(min_len, max_len)
    local len = math.random(min_len, max_len)
    -- Dùng các ký tự Latin Extended giống chữ thường
    local pool = {
...
local function gen_name(config, used)
    ...
    repeat
        tries = tries + 1
        if tries > 1000 then error("Không đủ tên để sinh — tăng name_length_max") end
...
function VariableMangling.process(code, config)
    ...
    -- 3. Gán tên mới theo (scope, varname)
    local scope_rename  = {}   -- scope_obj -> { old_name -> new_name }
    local used_globally = {}
